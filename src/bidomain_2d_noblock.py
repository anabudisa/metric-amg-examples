...
from bidomain_2d import setup_mms
...
def get_system(boundaries, data, pdegree, parameters):
    ...
    bcs = [DirichletBC(W.sub(0), u1, boundaries, tag) for tag in dirichlet_tags]
    bcs.extend([DirichletBC(W.sub(1), u2, boundaries, tag) for tag in dirichlet_tags])
...
if __name__ == '__main__':
    ...
    result_dir = f'./results/bidomain_2d_noblock/'
    ...
    for ncells in (2**i for i in range(4, 4+args.nrefs)):
        ...
        # OptDB.setValue('pc_hypre_boomeramg_strong_threshold', 0.25)
